...
const Services_1 = ({ price, plan, features, deliveryDetails, orderButton }) => {
  ...
  return (
    <div className='card md:h-[35rem] h-[38rem] sm:w-[22rem] w-[20rem] hover:-translate-y-1 hover:scale-105 duration-300'>
        <h1 className='text-gradient text-6xl font-bold'>{price}</h1>
        ...
        <button onClick={handleOrderClick} className={`my-4 ${orderButton.bgColor} hover:bg-white text-white hover:text-secondary py-1 px-16 hover:border-transparent rounded font-thin sm:text-[1rem] text-[0.8rem] w-[100%]`}>
            {orderButton.text}
        </button>
    </div>
  );
